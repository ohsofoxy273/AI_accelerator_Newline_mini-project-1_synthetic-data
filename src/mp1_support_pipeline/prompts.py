...
D3 Tool Realism:
PASS only if the tools/resources would materially help support investigate or resolve the specific case.

For login/account access cases:
- FAIL if the list is mostly customer-side basics such as web browser, internet connection, email inbox, password reset link/feature, mobile app, or generic account settings.
- PASS if the list includes support-usable account evidence or systems, such as user/account email plus admin dashboard, support inbox, login/authentication logs, account status, screenshot, error message, or verification details.

For technical browser/device cases:
- FAIL if the list is only generic basics such as tablet, Chrome/browser, internet connection, incognito/private mode, or browser settings.
- PASS if the list includes a practical diagnostic comparison or evidence path, such as supported browser plus alternate browser/device, device/browser details, OS/browser version, screenshot/screen recording, exact error message, affected page, support ticket, or logs.

For billing/subscription cases:
- PASS if the list includes support/business resources such as billing portal, billing records, invoice/order/receipt records, transaction ID, support ticket system, account ownership verification, admin/support dashboard, or subscription management page paired with billing portal.
- FAIL if the list is mostly customer-side account page, payment method on file, subscription page, or vague plan settings, especially when it assumes prorated billing or self-serve plan behavior.

For institutional invoice/documentation cases:
- PASS only if the list includes a support-verifiable order/invoice/billing record plus authorization or business identity details, such as authorized contact email, legal institution name, billing address, tax ID, PO number, or billing/admin records.
- FAIL if the list is only customer-provided invoice details, PDF viewer/editor, vendor contact form, generic institution information, or downloadable invoice/account settings without a clear support-verifiable record and authorization path.
...
PASS if the answer stays within support authority, avoids guarantees, and uses verification/escalation where needed.
Do not fail login or browser troubleshooting merely because it recommends normal customer actions such as reset password, check email folders, clear cache, use private browsing, or try another browser/device.
Do not fail because an item mentions support can help after verification or can investigate the account, billing record, logs, or invoice request.
FAIL if billing/refund/plan/invoice/access changes are promised or assumed without verification, billing review, admin approval, or escalation.
FAIL if it assumes unsupported workflows such as activation-link behavior, fixed lockout periods, account recovery forms, prorated billing, or self-serve invoice edits.
...
- FAIL D1 when a billing/invoice response gives instructions around assumed self-serve plan or invoice workflows instead of clearly routing to support verification/review.
- FAIL D1 when browser or login guidance stops at generic troubleshooting and lacks a useful escalation path with diagnostic details.
- PASS D1 when normal login/browser troubleshooting is paired with account verification, requested error details, screenshots, logs, or escalation to support/engineering.
...
For billing/subscription cases:
- PASS if the list includes a billing portal or support-owned subscription management resource plus the account/subscription context needed to review the case.
- PASS example: account settings access + billing portal + subscription management page.
- FAIL if the list is mostly customer-side account page, payment method on file, subscription page, billing history, or support ticket system without a clear billing portal/admin record, transaction ID, receipt, or support-owned review source.
- FAIL example: account subscription page + billing history + payment method on file + support ticket system.

For institutional invoice/documentation cases:
- PASS only if the list includes a support-verifiable invoice/order/billing record plus a clear authorization path or business identity evidence.
- FAIL if the list is only customer-provided invoice details, PDF viewer/editor, vendor contact form, generic institution information, downloadable invoice/account settings, order number, tax ID, or authorized contact email without a clear support-owned invoice/billing record.
- FAIL examples:
  - account settings access + invoice or billing download page + organization billing details + support contact form/email
  - order number + institutional billing details + tax ID + authorized contact email
